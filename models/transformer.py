    ...
    def __init__(self, **params):
        ...
        with self.init_scope():
            ...
            if activation == 'swish':
                ...
                self.activation = L.Swish(self.hidden_size, beta_init=1.0)
        if activation == 'swish':
            pass
...
class MultiHeadAttention(chainer.Chain):
    def __init__(self, **params):
        ...
        self.clip_distance = params.get('clip_distance', 8)
...
class Transformer(chainer.Chain):
    def __init__(self, vocab, **params):
        ...
        with self.init_scope():
            if self.relative_attention:
                self._init_relative_attention(**params)
    ...
    def _init_relative_attention(self, **params):
        K = params.get('clip_distance', 8)
        ...
        self.embed_relative_position = L.EmbedID(2 * K + 1, key_size, initialW=linear_init)
        #self.embed_relative_key_position = L.EmbedID(2 * K + 1, key_size, initialW=linear_init)
        self.embed_relative_key_position = self.embed_relative_position
        ...
        self.embed_relative_value_position = self.embed_relative_position
