...
from backport import adam as backport_adam
...
#default.train.final_learning_rate = 0.1
#default.train.final_learning_rate = 0.01
...
def build_optimizer(config, model):
    ...
    if optimizer_name in ['adam', 'amsgrad']:
        ...
        amsgrad = (optimizer_name is 'amsgrad')
        optimizer = optimizers.Adam(alpha=alpha, beta1=beta1, beta2=beta2, eps=eps, weight_decay_rate=weight_decay_rate, amsgrad=amsgrad)
    elif optimizer_name in ['adabound', 'amsbound']:
        alpha   = cdata.train.adam_alpha
        beta1   = cdata.train.adam_beta1
        beta2   = cdata.train.adam_beta2
        eps     = cdata.train.adam_eps
        gamma   = cdata.train.gamma
        #final_lr = cdata.train.final_learning_rate
        final_lr = cdata.train.sgd_learning_rate
        final_lr_rate = final_lr / alpha
        adabound = True
        #amsgrad = cdata.train.adam_amsgrad
        amsgrad = (optimizer_name is 'amsbound')
        if amsgrad:
        ...
        #self.optimizer = backport_adam.Adam(alpha=alpha, beta1=beta1, beta2=beta2, eps=eps, weight_decay_rate=weight_decay_rate, adabound=adabound)
        #self.optimizer = backport_adam.Adam(alpha=alpha, beta1=beta1, beta2=beta2, eps=eps, weight_decay_rate=weight_decay_rate, adabound=adabound, final_lr_rate=final_lr_rate)
        optimizer = backport_adam.Adam(alpha=alpha, beta1=beta1, beta2=beta2, eps=eps, gamma=gamma, weight_decay_rate=weight_decay_rate, adabound=adabound, amsgrad=amsgrad, final_lr_rate=final_lr_rate)
    else:
        # otherwise using SGD
        #cdata.optimizer = optimizer = 'sgd'
        cdata.train.optimizer = optimizer_name = 'sgd'
        optimizer = optimizers.SGD(lr=cdata.train.sgd_learning_rate)
    if comm:
    ...
    if weight_decay_rate > 0:
        if optimizer_name is 'sgd':
            optimizer.add_hook(chainer.optimizer.WeightDecay(weight_decay_rate))
    return optimizer

def get_record_name(model_path):
    found = re.findall('([^.]*).(npz|json)', model_path)
    if found:
        #record = found[0]
        record = found[0][0]
        if record == 'config':
            record = 'latest'
        return record
    raise ValueError("failed to guess record name of model path: {}".format(model_path))

def load_config(path, record):
    config = None
    ...
    if config is None:
        raise RuntimeError("failed to load config file: {}".format(config_path))
    return config

def load_model(path, record, config, Model):
    sp_model = os.path.join(path, 'sp.model')
    vocab = dataset.Vocabulary().load(sp_model)
    params = config.to_dict(flat=True)
    dprint(params)
    model = Model(vocab=vocab, **params)
...
def load_optimizer(path, record, config, model):
    optimizer = build_optimizer(config, model)
    optimizer_path = os.path.join(path, 'opt.params.{}.npz'.format(record))
    if os.path.exists(optimizer_path):
        try:
            infomain("loading optimizer from '{}' ...".format(optimizer_path))
            serializers.load_npz(optimizer_path, optimizer)
        except Exception as e:
            if comm_main:
                logger.error("failed to load: {}".format(optimizer_path))
                logger.error(repr(e))
            infomain("setting up an optimizer again")
            optimizer = build_optimizer(config, model)
    return optimizer
...
class Trainer(object):
    ...
    def increment_train_step(self):
        if chainer.config.train:
            ...
            if warmup_steps and warmup_steps > 0:
                ...
                elif cdata.train.optimizer in ['adabound', 'amsbound']:
                    alpha = train_factor * (embed_size ** -0.5) * min(train_step ** -0.5, train_step * warmup_steps ** -1.5)
                    self.optimizer.alpha = alpha
                    #final_lr_rate = cdata.train.final_learning_rate / alpha
                    final_lr_rate = cdata.train.sgd_learning_rate / alpha
                    self.optimizer.final_lr_rate = final_lr_rate
    ...
    def link_model(self, path, src_record, dist_record, log=True):
        ...
        for pattern in ["model.config.{}.json", "model.params.{}.npz", "opt.params.{}.npz"]:
    ...
    def load_status(self, path, record=None, model_path=None):
        ...
        if model_path is not None:
            if record is None:
                record = get_record_name(model_path)
        if record is None:
            record = 'latest'
        config = load_config(path, record)
        #sp_model = os.path.join(path, 'sp.model')
        #vocab = dataset.Vocabulary().load(sp_model)
        #params = config.to_dict(flat=True)
        #dprint(params)
        ...
        #infomain("loading model from '{}' ...".format(model_path))
        #serializers.load_npz(model_path, model)
        model = load_model(path, record, config, self.Model)
        optimizer = load_optimizer(path, record, config, model)
        self.config = config
        self.model = model
                ...
                if field in ['epoch']:
                    ...
                    if cdata.train.optimizer in ['adabound', 'amdbound']:
                        if cdata.train.warmup_steps == 0:
                            ...
                            lower = final_lr * (1.0 - 1.0 / (opt.gamma * opt.t + 1))
                            upper = final_lr * (1.0 + 1.0 / (opt.gamma * opt.t))
                            lr = lower
                        else:
            ...
            try:
                ...
                if xp is not np and isinstance(e, xp.cuda.memory.OutOfMemoryError):
                    if chainer.config.train:
                        ...
                        if new_batch_size >= cdata.train.min_batch_size:
                            cdata.train.batch_size = new_batch_size
                else:
                        logger.exception(e)
                continue
            ...
            if steps >= step_threshold:
                ...
                if args.filter_noise_gradually:
                    if train_size > 10 ** 5:
                        ...
                        noisy = carriculum_data[ (carriculum_data.feed_count >= 2) & ((carriculum_data.criterion-1) * (carriculum_data.last_step ** 0.5) * factor > vocab_size) ]
                        ...
                        if len(noisy) > 0:
                            ...
                            if comm_main:
                                logger.info("noisy training data (based on \"criteron vs training step\"):\n" + noisy_repr)
                ...
                if len(noisy) > 0:
                    ...
                    noisy_repr = repr(noisy[self.len_fields + ['cost', 'feed_count', 'last_epoch', 'last_step', 'criterion', 'priority']])
                    if comm_main:
                        logger.info("noisy training data (based on \"criterion vs feed count\":\n" + noisy_repr)
                        logger.info("dropping {} noisy training samples...".format(len(noisy)))
                    train_df.drop(noisy.index, inplace=True)
    ...
    def save_status(self, path, record=None):
        ...
        if record:
            model_name = 'model.params.{}.npz'.format(record)
            opt_name = 'opt.params.{}.npz'.format(record)
        else:
            model_name = 'model.params.npz'
            opt_name = 'opt.params.npz'
        model_path = os.path.join(path, model_name)
        opt_path   = os.path.join(path, opt_name)
        safe_remove(model_path, log=False)
        infomain("saving model into '{}' ...".format(model_path))
        serializers.save_npz(model_path, self.model)
        infomain("saving optimizer params into '{}' ...".format(opt_path))
        serializers.save_npz(opt_path, self.optimizer)
    ...
    def update_config(self, args):
        ...
        if self.model is None:
            ...
            if args.relative_attention:
                self.set_config('model.clip_distance', args.clip_distance)
    ...
    def create_parser(model_name):
        ...
        group.add_argument('--dev-files', '--dev', type=str, help='path to the validation data (tab separated values, or column-split files)', nargs='+')
        ...
        group.add_argument('--test-files', '--test', type=str, help='path to the evaluation data (tab separated values, or column-split files)', nargs='+')
        ...
        group.add_argument('--hidden-size', '--hidden', '--hs', '-H', type=int, default=None, help='Number of hidden layer nodes (default: 512 for S2SA, 2048 for Transformer')
        ...
        #group.add_argument('--relative_attention', '--relative', '--rel', type=strtobool, default=None, nargs='?', const=True, help='Using relative position representations for self-attention (default: {})'.format(default.model.relative_attention))
        ...
        group.add_argument('--recurrence', '--rec', type=str, default=None, choices=['basic', 'act', 'act-prod'], help='Auto-regression type for universal model (default: {})'.format(default.model.recurrence))
        ...
        group.add_argument('--batch-size', '--batch', '--bs', '-B', type=int, default=None, help='Size of Minibatch (default: {})'.format(default.train.batch_size))
        ...
        group.add_argument('--max-epochs', '--epochs', type=int, default=20, help='Max number of epochs (default: %(default)s)')
        ...
        group.add_argument('--optimizer', '-O', type=str, default=None, choices=['sgd', 'adam', 'amsgrad', 'adabound', 'amsbound'], help='Optimizer (default: {})'.format(default.train.optimizer))
        ...
        group.add_argument('--min-batch_size', '--min-batch', '--mb', type=int, default=None, help='Minimum batch size for fallbacking (default: {})'.format(default.train.min_batch_size))
        ...
        group.add_argument('--warmup-steps', '--warmup', '--ws', '-W', type=int, default=None, help='Size of warming up steps (default: {})'.format(default.train.warmup_steps))
        ...
        group.add_argument('--filter-noise-gradually', '--filter-noise', '--filter', type=strtobool, default=None, nargs='?', const=True, help='Filtering noisy training examples gradually with training steps')
        ...
        group.add_argument('--max-length', '--length', type=int, default=None, help='Maximum length (number of tokens) for training (default: {})'.format(default.train.max_length))
        return parser
